...
- 2026-06-24 **Zotcott 3.2.1**: search-query sanitisation. A query containing an apostrophe — `O'Brien` — crashed the search with `fts5: syntax error near "'"`, because the raw query was passed straight to FTS5's `MATCH`, whose operand is its own query language (`'` is the string delimiter; `"` `(` `)` `:` `-` `*` are also significant). `index.py` now routes every query through `sanitise_query()`, which splits on whitespace and wraps each term as a double-quoted FTS5 phrase (doubling embedded `"`), neutralising all special characters; the wildcard fallback became a proper `"term"*` prefix on the final token, and empty/punctuation-only input returns no results instead of raising. Covered by new `test_index.py` cases (apostrophe characterisation + a parametrised "never crash" guard over `'`, `(` `)`, `:`, `-`, `"`, `*`, empty).
